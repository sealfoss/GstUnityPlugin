...
extern "C" void VkTextureController_OnPluginLoad(IUnityInterfaces* interfaces)
{
...
UnityTextureController* CreateVkTextureController()
{
    return new VkTextureController(kUnityGfxRendererVulkan);
}

VkTextureController::VkTextureController(UnityGfxRenderer apiType)
    : m_UnityVulkan(NULL)
...
VkTextureController::~VkTextureController()
{
    DeleteBuffers();

    for (auto& pair : _texBuffers)
    {
        delete[] static_cast<VulkanBuffer*>(pair.second);
        pair.second = NULL;
    }
}

void VkTextureController::ProcessDeviceEvent(UnityGfxDeviceEventType type, IUnityInterfaces* interfaces)
{
    switch (type)
    {
...
bool VkTextureController::CreateVulkanBuffer(size_t sizeInBytes, VulkanBuffer* buffer, VkBufferUsageFlags usage)
{
    if (sizeInBytes == 0)
        return false;
...
void VkTextureController::ImmediateDestroyVulkanBuffer(const VulkanBuffer& buffer)
{
...
void VkTextureController::SafeDestroy(unsigned long long frameNumber, const VulkanBuffer& buffer)
{
    m_DeleteQueue[frameNumber].push_back(buffer);
}

void VkTextureController::GarbageCollect(bool force /*= false*/)
{
...
void VkTextureController::DrawSimpleTriangles(const float worldMatrix[16], int triangleCount, const void* verticesFloat3Byte4)
{
...
int VkTextureController::BeginModifyTexture(int id, void* texHandle, int texWidth,
    int texHeight, int texDepth)
{
    VulkanBuffer* vBuff = NULL;

    if (!_dimsSet)
    {
        _streamWidth = texWidth;
        _streamHeight = texHeight;
        _streamDepth = texDepth;
        _streamPitch = _streamWidth * _streamDepth;
        _texDataSize = _streamPitch * _streamHeight;
        _dimsSet = true;
    }

    if (_texData.find(id) == _texData.end())
    {
        const size_t stagingBufferSizeRequirements = _streamPitch * texHeight;
        UnityVulkanRecordingState recordingState;

        if (m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        {
            if (_texBuffers.find(id) != _texBuffers.end())
            {
                vBuff = (VulkanBuffer*)_texBuffers[id];
                SafeDestroy(recordingState.currentFrameNumber, *vBuff);
            }

            vBuff = new VulkanBuffer();
            _texBuffers[id] = (void*) vBuff;

            if (CreateVulkanBuffer(stagingBufferSizeRequirements, vBuff, VK_BUFFER_USAGE_TRANSFER_SRC_BIT))
                _texData[id] = vBuff->mapped;
...
void VkTextureController::EndModifyTexture(int id, void* textureHandle, unsigned char* dataPtr, int width, int height, int pitch)
{
    VulkanBuffer* vBuff = (VulkanBuffer*) _texBuffers[id];
    void* texData = _texData[id];
    memcpy(texData, dataPtr, _texDataSize);
        ...
        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_TRANSFER_WRITE_BIT, kUnityVulkanResourceAccess_PipelineBarrier, &image))
    {
        ...
        if (m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        {
            ...
            region.imageExtent.width = _streamWidth;
            region.imageExtent.height = _streamHeight;
            region.imageExtent.depth = 1;
            region.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
            region.imageSubresource.baseArrayLayer = 0;
            region.imageSubresource.layerCount = 1;
            region.imageSubresource.mipLevel = 0;
            vkCmdCopyBufferToImage(recordingState.commandBuffer, vBuff->buffer,
                image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
        }
    }
}

void* VkTextureController::BeginModifyVertexBuffer(void* bufferHandle, size_t* outBufferSize)
{
    UnityVulkanRecordingState recordingState;
    if (!m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        return NULL;
...
void VkTextureController::EndModifyVertexBuffer(void* bufferHandle)
{
